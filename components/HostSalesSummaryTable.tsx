...
const HostSalesSummaryTable: React.FC<HostSalesSummaryTableProps> = ({ summaryData, salesData }) => {
  ...
  return (
    <div className="mt-8">
      <h3 className="text-lg font-medium leading-6 text-white mb-4">Ringkasan Penjualan per Host</h3>
                    ...
                    return (
                        <React.Fragment key={host.hostId}>
                            <tr onClick={() => handleToggleExpand(host.hostId)} className="hover:bg-slate-700/50 cursor-pointer">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300">{host.totalSessions}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300">{formatDuration(host.totalDuration)}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300">Rp {host.totalNetTurnover.toLocaleString('id-ID')}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300">Rp {host.averageTurnover.toLocaleString('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</td>
                            </tr>
                            {isExpanded && (
                                <tr className="bg-slate-900/70">
                                    <td colSpan={6} className="p-0">
                                        <div className="p-4 mx-4 my-2 border-l-2 border-indigo-500">
                                            {hostSales.length > 0 ? (
